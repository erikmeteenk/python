...
def store_user(username, pwd):
    salt=os.urandom(32)
    hashed_password = create_hash(pwd,salt)
    ...
    if conn.is_connected():
        ...
        conn.commit()
        conn.close()
#store_user("erik1","Tombraider1")

def check_password(id, pwd):
    conn = create_connection()
    sql = (f'''select * from pwd_table where id = {id}''')
    cursor = conn.cursor()
    cursor.execute(sql)
    ...
    hex_hash = create_hash(pwd,salt)
    return hex_hash == hash_password_InDB


print(check_password(8, "Tombraider1"))
