...
def store_user(username, password):
    ...
    if conn.is_connected():
        ...
        conn.commit()
        conn.close()
#store_user("test3","TEST3")

def check_password(userid, userPassword):
    conn = create_connection()
    sql =   (f'''call get_user({userid});''')#(f'''select * from pwd_table where userid={userid}''')
    cursor = conn.cursor()
    print("trrrrrrrrrrrrrrrrrrrrrrrrrrrrr")
    cursor.execute (sql) 
    print("tes")
...
def get_all_users():
    conn = create_connection()
    sql = ('''select * from pwd_table''')#('''call get_all_users()''')
    cursor = conn.cursor()
    cursor.execute (sql) 
